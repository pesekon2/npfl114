...
class Network:
    ...
    def _build(self, args, num_words, num_tags, num_chars):
        ...
        embeddings_charseqs = tf.keras.layers.Embedding(input_dim=num_chars,
                                               ...
                                               mask_zero=False)(charseqs)

        mps = list()

        for width in range(2, args.cnn_max_width + 1):
            c1 = tf.keras.layers.Conv1D(filters=args.cnn_filters,
                                        kernel_size=width,
                                        strides=1,
                                        padding='valid',
                                        activation='relu')(embeddings_charseqs)
            mps.append(tf.keras.layers.GlobalMaxPool1D()(c1))

        mp = tf.keras.layers.concatenate(mps, axis=1)

        dense = tf.keras.layers.Dense(args.we_dim, activation='relu')(mp)
