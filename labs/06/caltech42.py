    ...
    class Dataset:
        ...
        def batches(self, size=None, image_processing=None):
            ...
            while len(permutation):
                ...
                # X = np.zeros(
                #     [batch_size, Caltech42.MIN_SIZE, Caltech42.MIN_SIZE,
                #      Caltech42.C], dtype=np.float32)
                # for i, index in enumerate(batch_perm):
                #     data = self._transformation(
                #         self._data["images"][index].copy())
                #
                #     if type(data) != np.ndarray:
                #         raise ValueError(
                #             "Caltech42: Expecting images after `transformation` to be Numpy `ndarray`")
                ...
                #                 (Caltech42.MIN_SIZE, Caltech42.MIN_SIZE,
                #                  Caltech42.C), np.float32))
                #     X[i] = data
                #
