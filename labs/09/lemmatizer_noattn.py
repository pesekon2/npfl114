...
class Network:
    def __init__(self, args, num_source_chars, num_target_chars):
        class Model(tf.keras.Model):
            def __init__(self):
                ...
                self._build(num_source_chars, num_target_chars, args)

                # def _build(self, num_source_chars, num_target_chars, args):
                # self.source_embeddings = tf.keras.layers.Embedding(
                #     input_dim=num_source_chars, output_dim=args.cle_dim,
                #     mask_zero=True)
                ...
                #     input_dim=num_target_chars, output_dim=args.cle_dim,
                #     mask_zero=False)  # TODO OP: mask_zero?
                # self.target_rnn_cell = tf.keras.layers.GRUCell(args.rnn_dim)
                # self.target_output_layer = tf.keras.layers.Dense(
                #     num_target_chars, activation="softmax")  # TODO OP: activation?

            def _build(self, num_source_chars, num_target_chars, args):
                self.source_embeddings = tf.keras.layers.Embedding(num_source_chars,
                                                   args.cle_dim, mask_zero=True)
                self.source_rnn = tf.keras.layers.Bidirectional(
                    tf.keras.layers.GRU(args.rnn_dim, return_sequences=False), merge_mode='sum')

                self.target_embeddings = tf.keras.layers.Embedding(num_target_chars,
                                                   args.cle_dim)
                self.target_rnn_cell = tf.keras.layers.GRUCell(args.rnn_dim)
                self.target_output_layer = tf.keras.layers.Dense(num_target_chars, activation=None)
    ...
    def _append_eow(self, sequences):
        ...
        sequences_rev_eow = tf.pad(sequences_rev, [[0, 0], [1, 0]],
                                   constant_values=MorphoDataset.Factor.EOW)
        return tf.reverse_sequence(sequences_rev_eow, tf.reduce_sum(
            tf.cast(tf.not_equal(sequences_rev_eow, 0), tf.int32), axis=1), 1)
    ...
    def train_batch(self, source_charseq_ids, source_charseqs,
                    target_charseq_ids, target_charseqs):
        ...
        with tf.GradientTape() as tape:
            ...
            embeddings = self._model.source_embeddings(source_charseqs)
            source_states = self._model.source_rnn(embeddings)
            ...
            class DecoderTraining(decoder.BaseDecoder):
                ...
                def initialize(self, layer_inputs, initial_state=None):
                    ...
                    # TODO OP: Type
                    inputs = self._model.target_embeddings(
                ...
                def step(self, time, inputs, states):
                    ...
                    # TODO OP: Source embeddings?
                    # finished = False
                    # if self._targets[time] is MorphoDataset.Factor.EOW:
                    #     finished = True
                    finished = tf.equal(self._targets[:, time],
    ...
    def train_epoch(self, dataset, args):
        for batch in dataset.batches(args.batch_size):
            ...
            # TODO OP: LEMMAS? TAGS?
            predictions = self.train_batch(batch[dataset.FORMS].charseq_ids,
            ...
            for i in range(len(batch[dataset.LEMMAS].charseqs[1])):
                if batch[dataset.LEMMAS].charseqs[1][i]:
                    gold_lemma += dataset.data[dataset.LEMMAS].alphabet[
                        batch[dataset.LEMMAS].charseqs[1][i]]
                    system_lemma += dataset.data[dataset.LEMMAS].alphabet[
                        predictions[0][i]]
            print(float(self._metrics_training["accuracy"].result()), form,
                  gold_lemma, system_lemma)

    @tf.function(
        input_signature=[tf.TensorSpec(shape=[None, None], dtype=tf.int32)] * 2,
        autograph=False)
    def predict_batch(self, source_charseq_ids, source_charseqs):
        ...
        class DecoderPrediction(decoder.BaseDecoder):
            @property
            def batch_size(
                    self): return tf.shape(self._source_states)[0]  # TODO: Return batch size of self._source_states, using tf.shape

            @property
            def output_size(
                    self): return 1  # TODO: Return 1 because we are returning directly the predictions

            @property
            def output_dtype(
                    self): return tf.int32  # TODO: Return tf.int32 because the predictions are integral
            ...
            def step(self, time, inputs, states):
                ...
                #     self._model.target_output_layer(outputs), axis=2,
        ...
        predictions, _, _ = DecoderPrediction(
            maximum_iterations=tf.shape(source_charseqs)[1] + 10)(
            [self._model, source_states])
        return predictions

    @tf.function(
        input_signature=[tf.TensorSpec(shape=[None, None], dtype=tf.int32)] * 4,
        autograph=False)
    def evaluate_batch(self, source_charseq_ids, source_charseqs,
                       target_charseq_ids, target_charseqs):
        # Predict
        ...
        target_charseqs = self._append_eow(target_charseqs)
        targets = tf.boolean_mask(
            tf.gather(target_charseqs, target_charseq_ids),
            tf.not_equal(source_charseq_ids, 0))

        # Compute accuracy, but on the whole sequences
        mask = tf.cast(tf.not_equal(targets, 0), tf.int32)
        resized_predictions = tf.concat([predictions, tf.zeros_like(targets)],
                                        axis=1)[:, :tf.shape(targets)[1]]
    ...
    def evaluate(self, dataset, dataset_name, args):
        for metric in self._metrics_evaluation.values():
            metric.reset_states()
        for batch in dataset.batches(args.batch_size):
            predictions = self.evaluate_batch(batch[dataset.FORMS].charseq_ids,
                                              batch[dataset.FORMS].charseqs,
                                              batch[dataset.LEMMAS].charseq_ids,
                                              batch[dataset.LEMMAS].charseqs)

        metrics = {name: float(metric.result()) for name, metric in
                   self._metrics_evaluation.items()}
        with self._writer.as_default():
...
if __name__ == "__main__":
    ...
    args.logdir = os.path.join("logs", "{}-{}-{}".format(
        ...
        ",".join(("{}={}".format(re.sub("(.)[^_]*_?", r"\1", key), value) for
                  key, value in sorted(vars(args).items())))
    ))
    ...
    network = Network(args,
                      ...
                      num_target_chars=len(
                          morpho.train.data[morpho.train.LEMMAS].alphabet))
    for epoch in range(args.epochs):
        network.train_epoch(morpho.train, args)
        metrics = network.evaluate(morpho.dev, "dev", args)
        print(
            "Evaluation on {}, epoch {}: {}".format("dev", epoch + 1, metrics))
